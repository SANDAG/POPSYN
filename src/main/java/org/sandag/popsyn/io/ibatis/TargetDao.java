        ...
        implements ITargetDao
{
            ...
            Version version)
    {
        ...
        for (Geography zone : zones)
        {
            ...
            result.add((Target) getSqlMapClientTemplate().queryForObject("Target.getTargetByZone",
                    params));
        }
        return result;
    }
    ...
    public double getZonalHHTotal(int zoneId, Version version)
    {
        ...
        Target t = (Target) getSqlMapClientTemplate().queryForObject("Target.getTargetByZone",
                param);
        if (t != null)
        {
            result = t.getWeight();
            ...
            List<IControlDefinition<T>> ctrlDefs, Version version)
    {
        ...
        for (IControlDefinition<T> ctrlDef : ctrlDefs)
        {
            ...
            Double returnVal = (Double) getSqlMapClientTemplate().queryForObject(
                    "Target.getTargetSummation", param);
            ...
            List<IControlDefinition<T>> ctrlDefs, Version version)
    {
        ...
        {
            param.setCategory(ctrlDef.getCategory());
            Double val = (Double) getSqlMapClientTemplate().queryForObject(
                    "Target.getTargetByZone2", param);
    ...
    {
        getSqlMapClientTemplate().queryForObject("Target.regenerateTargets", tyear);
    }

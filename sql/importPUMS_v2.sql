...
--Updated to read PUMA list automatically
--Sriram Narayanamoorthy, narayanamoorthys@pbworld.com, 122014
--------------------------------------------------------------------
...
SET @query = ('BULK INSERT dbo.psam_h11 FROM ' + '''' + @pumsHH_File + '''' + ' WITH (FIELDTERMINATOR = ' + 
...
SET @query = ('BULK INSERT dbo.psam_p11 FROM ' + '''' + @pumsPersons_File + '''' + ' WITH (FIELDTERMINATOR = ' + 
				''',''' + ', ROWTERMINATOR = ' + '''\n''' + ', FIRSTROW = 2, MAXERRORS = 0, TABLOCK);');
EXEC(@query);
...
SET @query = 'SELECT * INTO dbo.tempHH FROM dbo.psam_h11 WHERE PUMA IN (' + @pumaList + ')';
EXEC(@query);

SET @query = 'SELECT * INTO dbo.tempPer FROM dbo.psam_p11 WHERE PUMA IN (' + @pumaList + ')';
EXEC(@query)
...
SELECT * INTO #tempHH FROM dbo.tempHH
DROP TABLE dbo.tempHH

SELECT * INTO #tempPer FROM dbo.tempPer
...
------------------------------------------------------------------------------------
SELECT * INTO dbo.hhtable FROM #tempHH
SELECT * INTO dbo.perstable FROM #tempPer
...
--Loading matching PUMA records into a temporary table
SET @query = 'SELECT * INTO tempHH2 FROM dbo.psam_h11 WHERE PUMA IN (' + @pumaList + ')';
EXEC(@query);

SET @query = 'SELECT * INTO tempPer2 FROM dbo.psam_p11 WHERE PUMA IN (' + @pumaList + ')';
EXEC(@query)

--#temptables created using dynamic SQL has a diferent scope and can't be referenced by the main SQL script
--HACK: Transferring data between a normal and a temporary table and dropping the normal table
--TODO: Create #temp table body and use INSERT syntax
SELECT * INTO #tempHH2 FROM dbo.tempHH2
DROP TABLE dbo.tempHH2

SELECT * INTO #tempPer2 FROM dbo.tempPer2
DROP TABLE dbo.tempPer2
...
------------------------------------------------------------------------------------
SELECT * INTO dbo.gqhhtable FROM #tempHH2
SELECT * INTO dbo.gqperstable FROM #tempPer2
